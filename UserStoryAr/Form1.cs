...
namespace UserStoryAir
{
    ...
    {
        private NugetAir.ReysWF<Reys> reyslib;
        ...
        public Аэропорт()
        {
            ...
            reys = new List<Reys>();
            reyslib = new NugetAir.ReysWF<Reys>();
        ...
        private void ChangeTool_Click(object sender, EventArgs e)
        {
            ...
            if (infoform.ShowDialog(this) == DialogResult.OK)
            {
                ...
                bindingSource.ResetBindings(false);
                reyslib.Change(data, infoform.Reys);
                CalculateStats();
            }
        }
        ...
        public void CalculateStats()
        {
            ...
            РейсыStripStatusLabel1.Text = $"Кол-во рейсов: " + count;
                pas += reyslib.GetList().Sum(x => x.NumberPassengers);
                Ekip += reyslib.GetList().Sum(x => x.NumberCrew); 
                Summa += reyslib.GetList().Sum(x => (x.NumberPassengers * x.SborP + x.NumberCrew * x.SborC) * ((100.00m + x.allowance) / 100.0m));
